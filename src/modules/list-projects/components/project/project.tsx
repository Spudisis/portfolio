...
export const Project = ({ id, preview, github, href }: ObjectType) => {
	const { t } = useTranslation();
	return (
		<div className={s.cart}>
			<img className={s.image} src={preview} height={200} alt="screen site" />
			<h3 className={s.name}>{t(`projects.project.${id}.name`)}</h3>
			<Link target={github ? "_blank": undefined} to={github ? href :PATHS.PROJECTS + "/" + id} relative="path" className={s.button}>
				{t("More")}
			</Link>
		</div>
	);
